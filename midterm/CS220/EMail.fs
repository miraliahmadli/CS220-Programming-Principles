...
module EMail =
  ...
  let create (str: string) = 
    ...
    if str.Length>319 || str.Length=0 || isDD(int str.[0]) || isDD(int str.[str.Length-1]) then None
    else 
        let rec check (str: string) c1 c2 max At= 
            if c2=max then (c1,c2)
            elif At then 
                if isV(int str.[c2]) then check str c1 (c2+1) max At
                else (-1,-1)
            else 
                if c1=max then (-1,-1)
        ...
        if a= -1 || a+1=str.Length || a=0 || isDD(int str.[a-1]) || isDD(int str.[a+1]) then None
